...
inline void meregeVertices(Mesh * m){
	...
	std::vector< std::vector<typename Mesh::Vertex> > faces;
	for (auto f : m->faces()){
		std::vector<typename Mesh::Vertex> face;
		for (auto v : m->vertices(f)) face.push_back(typename Mesh::Vertex(xrefs[v.idx()]));
